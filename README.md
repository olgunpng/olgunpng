<div align="center">

<img src="https://capsule-render.vercel.app/api?type=venom&color=0:0F2027,50:203A43,100:2C5364&height=280&section=header&animation=fadeIn" width="100%"/>

<br/>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=30&duration=2500&pause=800&color=6C7BE0&center=true&vCenter=true&width=700&lines=%24+whoami;%3E+olgun_eser;%24+systemctl+status+pipelines;%3E+%E2%97%8F+active+(running)" alt="terminal typing"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=400&size=16&duration=3000&pause=1000&color=8A93A8&center=true&vCenter=true&width=650&lines=n8n+%2B+Supabase+%2B+LLM+destekli+prod%C3%BCksiyon+sistemleri;Demo+de%C4%9Fil%2C+ger%C3%A7ek+trafik+alan+%C3%BCr%C3%BCnler;Otomasyon+%C2%B7+Backend+%C2%B7+AI+Entegrasyonu" alt="tagline typing"/>

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=olgunpng&label=PROFILE+VIEWS&color=6C7BE0&style=for-the-badge" alt="profile views"/>
<img src="https://img.shields.io/badge/STATUS-BUILDING%20IN%20PROD-2E3192?style=for-the-badge" alt="status"/>

</div>

<br/>

## 🧠 Hakkımda

> ✍️ **👋 Merhaba, ben Olgun. Otomasyon, web geliştirme ve siber güvenlik alanlarında çalışıyorum; şu anda tamamen AI ile çalışan bir haber sitesi olan Yöntem Haber'i geliştirip işletiyorum. n8n, Supabase ve çeşitli LLM API'leri (Groq, Gemini) ile üretim seviyesinde otomasyon sistemleri kurmaya odaklanıyorum. Aynı zamanda freelance Full-Stack Dev. olarak çalışıyorum.**

- 🔭 **Şu an üzerinde çalıştığım:** `Yöntem Haber, LLM Pipeline'ları, Self-Hosted Agent sistemleri ve Siber Güvenlik`
- 💬 **Bana şu konularda ulaşabilirsin:** `LLM tabanlı sistemler (n8n otomasyonu, self-hosted AI agent'lar, AI destekli içerik pipeline'ları) ve web geliştirme`
- 📍 **Konum:** `Istanbul/Türkiye`

---

## 🧰 Tech Stack

<div align="center">

**Frontend**
<br/>
<img src="https://skillicons.dev/icons?i=html,css,js,ts,jquery,react,bootstrap,tailwind,vue" />

**Backend & Framework**
<br/>
<img src="https://skillicons.dev/icons?i=nodejs,express,nextjs" />

**Veritabanı**
<br/>
<img src="https://skillicons.dev/icons?i=supabase,postgres,mongodb" />

**Deploy & Hosting**
<br/>
<img src="https://skillicons.dev/icons?i=vercel,netlify" />

**Otomasyon & AI**
<br/>
<img src="https://img.shields.io/badge/n8n-EA4B71?style=for-the-badge&logo=n8n&logoColor=white" />
<img src="https://img.shields.io/badge/Cloudinary-3448C5?style=for-the-badge&logo=cloudinary&logoColor=white" />
<img src="https://img.shields.io/badge/Groq-F55036?style=for-the-badge&logoColor=white" />
<img src="https://img.shields.io/badge/DeepInfra-1A1A2E?style=for-the-badge&logoColor=white" />

**CMS & Araçlar**
<br/>
<img src="https://skillicons.dev/icons?i=wordpress,figma,git,docker" />

</div>

---

## 📌 Öne Çıkan Projeler

> `| [emoji] **[Proje Adı]** | [Kısa açıklama] | ![Tech](https://img.shields.io/badge/-Tech-black) | [🔗 Repo/Site](link) |`

| Proje | Açıklama | Stack | Link |
|---|---|---|---|
| `Yöntem Haber` | `Tamamen AI ile çalışan bir haber platformudur.` | `n8n, Supabase, Cloudinary, Groq, Gemini API` | `https://www.yontemhaber.com` |
| `Yeşil Evren` | `Bilim/teknoloji forum sitesidir.` | `Next.js, Supabase, PostgreSQL, Tailwind CSS, Vercel` | `https://www.yesilevren.net` |
| `Ziyalılar` | `Online dergi sitesidir.` | `ReactJS, Supabase` | `https://www.ziyalilar.org` |
| `Stack up Stack!` | `Basit ve bağımlılık yaratan bir stacking/kule dizme mobil oyunu.` | `Expo (React Native), Supabase` | `https://github.com/olgunpng/stackbystack-legal` |
| `Poppfy` | `Kayıt şirketleri arasında anlık fiyat karşılaştırması yapan AI destekli domain öneren, size uygun domaini bulmanıza yardımcı olan bir uygulama.` | `Next.js, React, Supabase, PostgreSQL, Tailwind CSS, Three.js, GSAP, Vercel` | `https://www.poppfy.com` |
| `BlueSky Takibi Bırakanlar` | `BlueSky için takibi bırakanlar uygulaması, ` | `ReactJS, MongoDB, Bootstrap, Vercel` | `https://github.com/olgunpng/blueskytakipci` |


---

## 📊 GitHub İstatistikleri

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=olgunpng&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=olgunpng&layout=compact&theme=tokyonight&hide_border=true" />

<img src="https://github-readme-streak-stats.herokuapp.com/?user=olgunpng&theme=tokyonight&hide_border=true" />

</div>

---

## 🏆 Trophies

<div align="center">
<img src="https://github-profile-trophy.vercel.app/?username=olgunpng&theme=tokyonight&no-frame=true&row=1&column=6" />
</div>

---

## 📈 Activity Graph

<div align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=olgunpng&theme=tokyo-night&hide_border=true" />
</div>

---

## 🐍 Contribution Snake

<div align="center">
<img src="https://raw.githubusercontent.com/olgunpng/olgunpng/output/github-contribution-grid-snake.svg" alt="snake animation" />
</div>

---

## 📫 İletişim

<div align="center">

<a href="mailto:BURAYA_MAIL"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
<a href="https://github.com/olgunpng"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/></a>
<a href="[BURAYA_LINKEDIN](https://www.linkedin.com/in/olgun-eser-duman/)"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>

</div>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:2C5364,50:203A43,100:0F2027&height=100&section=footer" width="100%"/>

</div>
